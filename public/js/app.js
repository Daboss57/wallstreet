...
const App = {
  ...
  route() {
    ...
    switch (hash) {
      ...
      case '#/news':
        ...
        break;
      case '#/firms':
        this.currentPage = Firms;
        Firms.render(container);
        break;
      default:
        window.location.hash = '#/';
    }
  ...
  async resetAccount() {
    if (!confirm('ARE YOU SURE? This will wipe all your trades, orders, and reset your cash to $100,000. This cannot be undone.')) return;

    try {
      await Utils.post('/auth/reset', {});
      Utils.showToast('success', 'Account Reset', 'You are back to square one.');
      window.location.reload();
    } catch (e) {
      Utils.showToast('error', 'Reset Failed', e.message);
    }
  }
};
